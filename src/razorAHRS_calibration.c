  ...
  while (!stopRead) {
    ...
    if (result == 1) {
      if (data->calibTypeSet == false) {
        ...
        if ((singleByte == 'a') || (singleByte == 'm') || (singleByte == 'g')) {
        ...
        if (bufferInput == 2) {
          ...
          if (strcmp (stringBuffer, "ac") == 0)
            data->calibType = 0;
          else if (strcmp (stringBuffer, "mg") == 0)
            data->calibType = 1;
          else if (strcmp (stringBuffer, "gy") == 0)
            data->calibType = 2;
          else
            data->calibTypeSet = false;
          bufferInput = 0;
...
//                      if(data->calibType == 0){
      ...
      if (headerSet[0] == false) {
        ...
        if (step <= 7) {
          ...
          printf (" %6.1f | %6.1f || %6.1f | %6.1f || %6.1f | %6.1f \r", data->values[0], data->values[1], data->values[2], data->values[3], data->values[4], data->values[5]);
        }
        ...
        if (keepValue == true) {
          ...
          keepValue = false;
          if (step == 0)
            data->acc_xmin = data->values[0];
          else if (step == 1)
            data->acc_xmax = data->values[1];
          else if (step == 2)
            data->acc_ymin = data->values[2];
          else if (step == 3)
            data->acc_ymax = data->values[3];
          else if (step == 4)
            data->acc_zmin = data->values[4];
          else if (step == 5)
            data->acc_zmin = data->values[5];
          //else if(step == 6) {}
          else if (step == 7) {
    ...
    if (read (STDIN_FILENO, &console, 1) > 0) {
      ...
      //else if((console == 'n') ||  (console == 'N')) write(settings->tty_fd,"#on",3);
        ...
        if (step == 5) {
          ...
          write (settings->tty_fd, "#oc", 3);   // restart callibration (inner mode)
        razorSleep (20);
        step++;
      }
    }
